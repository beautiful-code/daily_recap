class DailyLog < ApplicationRecord
  ...
  include ApplicationHelper
	def create_log (user_id,takeaway,log_date)
    #TODO save daily logs and its log entries in a active record transaction
    ActiveRecord::Base.transaction do
	   DailyLog.create(log_date: log_date,user_id: user_id,takeaway: takeaway )
    end
  ...
  def create_record()
      ...
      #summary_record[:name] = current_user.name
      #summary_record[:name]=User.find(key.user_id)[0].name
      ...
      summary_record[:logdate]=self.log_date
      summary_record
  end
  def company_record()
  	company_record={}
